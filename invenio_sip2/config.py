...
from invenio_sip2.actions import (
...
SIP2_MESSAGE_ACTIONS = {
    ...
    "17": dict(message="item_information", response="18", action=ItemInformation),
    "19": dict(message="item_status_update", response="20", action=ItemStatusUpdate),
    "23": dict(message="patron_request_status", response="24", action=PatronStatus),
    "25": dict(message="patron_enable", response="26", action=PatronEnable),
    "29": dict(message="renew", response="30", action=Renew),
    "35": dict(message="end_patron_session", response="36", action=EndPatronSession),
    "37": dict(message="fee_paid", response="38", action=FeePaid),
    "63": dict(message="patron_information", response="64", action=PatronInformation),
    ...
    "99": dict(
        message="sc_status", response="98", action=AutomatedCirculationSystemStatus
    ),
}
...
SIP2_FIXED_FIELD_DEFINITION = dict(
    available=dict(length=1, label=_("available"), callback=convert_bool_to_char),
    ...
    payment_accepted=dict(
        length=1, label=_("payment accepted"), callback=convert_bool_to_char
    ),
    ...
    end_session=dict(length=1, label=_("End Session"), callback=convert_bool_to_char),
    ...
    unavailable_holds_count=dict(
        length=4, fill="0", label=_("unavailable holds count")
    ),
    ...
    renewal_ok=dict(length=1, label=_("renewal ok"), callback=convert_bool_to_char),
    unrenewed_count=dict(length=4, label=_("renewal ok")),
    renewed_count=dict(length=4, label=_("renewal ok")),
    magnetic_media=dict(length=1, label=_("magnetic media"), callback=convert_to_char),
    desensitize=dict(length=1, label=_("desensitize"), callback=convert_to_char),
    resensitize=dict(length=1, label=_("resensitize"), callback=convert_bool_to_char),
...
SIP2_VARIABLE_FIELD_DEFINITION = dict(
    ...
    screen_messages=dict(field_id="AF", multiple=True, label=_("screen message")),
    ...
    valid_patron=dict(
        field_id="BL", length=1, label=_("valid patron"), callback=convert_bool_to_char
    ),
    renewed_items=dict(field_id="BM", multiple=True, label=_("renewed items")),
    unrenewed_items=dict(field_id="BN", multiple=True, label=_("unrenewed items")),
    ...
    hold_items_limit=dict(
        field_id="BZ", length=4, fill="0", label=_("hold items limit")
    ),
    overdue_items_limit=dict(
        field_id="CA", length=4, fill="0", label=_("overdue items limit")
    ),
    charged_items_limit=dict(
        field_id="CB", length=4, fill="0", label=_("charged items limit")
    ),
    fee_limit=dict(field_id="CC", label=_("fee limit")),
    unavailable_hold_items=dict(
        field_id="CD", multiple=True, label=_("unavailable hold items")
    ),
    ...
    media_type=dict(
        field_id="CK", length=3, label=_("media type"), callback=get_media_type
    ),
