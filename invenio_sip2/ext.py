...
def load_fixed_field(app):
    ...
    for name, field in app.config["SIP2_FIXED_FIELD_DEFINITION"].items():
        setattr(
            MessageTypeFixedField, name, MessageTypeFixedField(name=name, field=field)
        )


def load_variable_field(app):
    """Load variable field configuration."""
    for name, field in app.config["SIP2_VARIABLE_FIELD_DEFINITION"].items():
        setattr(
            MessageTypeVariableField,
            name,
            MessageTypeVariableField(name=name, field=field),
        )
...
class InvenioSIP2(object):
    ...
    def init_app(self, app):
        ...
        # Set SIP2 datastore
        datastore_class = obj_or_import_string(app.config["SIP2_DATASTORE_HANDLER"])
    ...
    def add_console_handler(self, app):
        ...
        self._add_logger_handler(handler, app.config["SIP2_LOGGING_CONSOLE_LEVEL"])
    ...
    def sip2(self):
        ...
        return _SIP2(action_config=deepcopy(current_app.config["SIP2_MESSAGE_ACTIONS"]))
    ...
    def support_checkin(self):
        ...
        return convert_bool_to_char(current_app.config["SIP2_SUPPORT_CHECKIN"])

    @cached_property
    def support_checkout(self):
        """Support of checkout by the automated circulation system."""
        return convert_bool_to_char(current_app.config["SIP2_SUPPORT_CHECKOUT"])

    @cached_property
    def support_online_status(self):
        """Support of online status by the automated circulation system."""
        return convert_bool_to_char(current_app.config["SIP2_SUPPORT_ONLINE_STATUS"])

    @cached_property
    def support_offline_status(self):
        """Support of offline status by the automated circulation system."""
        return convert_bool_to_char(current_app.config["SIP2_SUPPORT_OFFLINE_STATUS"])
